  ...
  return (
    <div className="border-2 border-black w-64 h-64 rounded-md ml-2">
      ...
      {bodyParts[chosenBodyPart - 1].files.map(
        (file, index) => (
            <div key={index} onClick={() => handleImage(file.secure_url)} className="bg-zinc-400 hover:bg-slate-100 rounded-sm mb-0.5">
                <p className="ml-2 text-white hover:text-black">{file.display_name}</p>
