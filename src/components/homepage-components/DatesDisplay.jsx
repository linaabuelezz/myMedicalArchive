  ...
  return (
    <div className="border-2 border-black w-64 h-64 rounded-md ml-4">
      ...
      {bodyParts[chosenBodyPart - 1].dates.map(
        (date, index) => (
            <div key={index} onClick={() => handleDateClick(date)} className="bg-zinc-400 rounded-sm hover:bg-slate-100 mb-0.5">
                <p className="ml-2 text-white hover:text-black">{date.newEventName}</p>
            </div>
        )
      )}
    </div>
