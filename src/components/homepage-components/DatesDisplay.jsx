...
const DatesDisplay = () => {
  ...
  return (
    <div className="bg-white border-2 border-black w-80 h-80 rounded-md ml-4">
      ...
      {bodyParts[chosenBodyPart - 1].dates.map((date, index) => (
        <div
          key={index}
          onClick={() => handleDateClick(date)}
          className="bg-zinc-400 rounded-sm hover:bg-slate-100 mb-0.5 hover:cursor-pointer"
        >
          <p className="ml-2 text-white hover:text-black font-mono">
            {date.newEventName}
          </p>
        </div>
      ))}
